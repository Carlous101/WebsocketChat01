...
public class MiChat {
    ...
    //Envia el mensaje a todas las personas que esten conectadas..
    @OnMessage
    public void mensaje(Mensaje mensaje) throws IOException, EncodeException{
        for(Session sesion:conectados){
            sesion.getBasicRemote().sendObject(mensaje);//Se le envia el mensaje, para que el cliente procese la inf
        }
    }
}
